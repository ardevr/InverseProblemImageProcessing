...
class Experience(object) :
    ...
    def __init__(self, T = 100, 
        ...
        if F is None and F_hat is None:
            ...
            self.F = lambda x : np.exp(-x**2 / 4) * (2 - x**2) / (4 * np.sqrt(2))
        else :
            self.F_hat = F_hat
            self.F = F
    ...
    def emp_cross_correlation(self,x1,x2, T = None, tau = None) :
        ...
        emp_cross_cor = 1/(T-time_dicretization[0])*np.sum(u*u_lagged*(time_dicretization[1::] - time_dicretization))
    ...
    def exp_emp_cross_correlation(self,x1,x2, tau = None, N = None) :
        ...
        temp_cor = 0
        for s in range(N) : 
            f = lambda w : self.F_hat(w)*np.conjugate(G_hat(w,x1,y[s]))*G_hat(w,x2,y[s])*np.exp(-1j*w*tau)
    ...
    def cross_1(self, x1, x2 ,tau = None, L = None) :
        ...
        if tau is None :
            tau = self.tau
        G_hat = lambda w, x, theta : self.env.G_hat(w, x, L*np.array([np.cos(theta), np.sin(theta)]))
        f = lambda theta : quad(lambda w : self.F_hat(w)*np.conjugate(G_hat(w,x1, theta))*
    ...
    def C_asy(self,x1, x2, tau = None) :
        ...
        L = self.env_parameters['L']
        G = lambda t : self.env.G(t,x1,x2)
        G_rev = lambda t : self.env.G(-t,x1,x2)
        dC_asy = lambda tau : - c0/(8*np.pi**2*L)*(np.convole(self.F,G)(tau) - np.convole(self.F,G_rev)(tau))
