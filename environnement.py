...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  1 08:21:34 2019
...
def generate_n_gp(time_points, return_fourier=True):
    ...
    def f(x): return np.exp(-x**2 / 4) * (2 - x**2) / (4 * np.sqrt(2))
    tau = time_points[-1] / (len(time_points) - 1)
    burn_in = int(10 / tau)
    temp_points = np.linspace(-10,
                              time_points[-1] + 10,
                              2 * burn_in + len(time_points))
...
class Environnement():
    ...
    def G_hat(self, w, x, y):
        ...
        if np.abs(w) > 0:
            return 1j / 4 * np.sign(w) * j0(np.abs(w) * norme) - \
                1 / 4 * np.nan_to_num(y0(np.abs(w) * norme))
        else:
            return 1j / 4
